...
resource "aws_s3_bucket" "static_hosting" {
  bucket        = "${var.prefix}-static-hosting-pipeline-${data.aws_caller_identity.self.account_id}"
  force_destroy = true
}
...
resource "aws_s3_bucket" "static_hosting_artifact" {
  bucket        = "${var.prefix}-static-hosting-pipeline-artifact-${data.aws_caller_identity.self.account_id}"
  force_destroy = true
}
